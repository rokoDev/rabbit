...
TEST(SerializeTests, UInt16)
{
    ...
    ASSERT_TRUE(writer::is_success(rabbit::serialize(w, value)));
    ASSERT_EQ(w.pos(), bit_pos(16));

    reader r{rawData};
    std::uint16_t deserialized_value{};
    ASSERT_TRUE(reader::is_success(rabbit::deserialize(r, deserialized_value)));
...
TEST(SerializeTests, Empty)
{
    ...
    writer w{rawData};
    Empty valueToSave;
    ASSERT_TRUE(writer::is_success(rabbit::serialize(w, valueToSave)));
